...
### 7. DotDict - Enhanced Dictionary (`cloudmesh.ai.common.dotdict`)
A powerful dictionary subclass that supports both attribute-style access and dot-notation for working with nested configuration data.

- **Dual Access**: Access values via `config.key` (attribute) or `config["key"]` (bracket).
- **Deep Nesting**: Navigate nested structures with dot notation like `config["level1.level2.key"]`.
- **Auto-Conversion**: Standard dictionaries are automatically converted to DotDict recursively.
- **Placeholder Expansion**: Replace `{variable}` placeholders with values from the dict.
- **Deep Merging**: Merge configurations with recursive dictionary merging.
- **Smart Lookup**: `smart_get()` searches recursively for keys when not found directly.
- **Serialization**: Built-in YAML and JSON output with support for multi-line strings.
- **Ordered**: Maintains insertion order via OrderedDict base class.

## Usage Examples

### DotDict - Enhanced Dictionary
```python
from cloudmesh.ai.common.dotdict import DotDict

# Create from dict or kwargs
config = DotDict({"cloudmesh": {"ai": {"server": "uva"}}})

# Attribute access (chaining)
print(config.cloudmesh.ai.server)  # 'uva'

# Dot-notation bracket access
print(config["cloudmesh.ai.server"])  # 'uva'

# Safe access with defaults (supports dot-notation)
print(config.get("cloudmesh.ai.server"))  # 'uva'
print(config.get("missing.key", "default"))  # 'default'

# Smart get - searches recursively for keys
print(config.smart_get("server"))  # 'uva'

# Assignment with auto-creation of nested structures
config["cloudmesh.ai.port"] = 8000
config.version = "2.0"

# Deep merging
config.merge({"training": {"epochs": 100, "lr": 0.001}})
print(config.training.epochs)  # 100

# Placeholder expansion
config = DotDict({"name": "gemma", "path": "/models/{name}"})
expanded = config.expand()
print(expanded.path)  # '/models/gemma'
...
expanded = config.expand()
print(expanded.path)  # '/home/user/models'

# Serialization
config = DotDict({"a": {"b": 1}})
print(config.to_json(indent=2))
print(config.yaml)  # YAML output with literal blocks for multi-line strings
```
