...
class TestGPUDetection(unittest.TestCase):
    ...
    def test_nvidia_detection(self, mock_sub):
        ...
        self.assertTrue(info["gpu.present"])
        self.assertEqual(info["gpu.vendor"], "NVIDIA")
        self.assertIn("16.0 GiB", info["gpu.total_vram"])

    @patch('subprocess.check_output')
    def test_amd_detection(self, mock_sub):
        ...
        with patch('cloudmesh.ai.common.sys._get_nvidia_gpu_info', return_value={"gpu.present": False}):
            info = ai_sys.get_gpu_info()
            self.assertTrue(info["gpu.present"])
            self.assertEqual(info["gpu.vendor"], "AMD")

    @patch('cloudmesh.ai.common.sys.os_is_mac', return_value=True)
    @patch('subprocess.check_output')
    def test_apple_detection(self, mock_sub, mock_mac):
        ...
        with patch('cloudmesh.ai.common.sys._get_nvidia_gpu_info', return_value={"gpu.present": False}), \
            ...
            info = ai_sys.get_gpu_info()
            self.assertTrue(info["gpu.present"])
            self.assertEqual(info["gpu.vendor"], "Apple")
            self.assertEqual(info["gpu.note"], "Unified Memory")

    @patch('cloudmesh.ai.common.sys.os_is_linux', return_value=True)
    @patch('cloudmesh.ai.common.sys.os_is_mac', return_value=False)
    def test_intel_detection(self, mock_mac, mock_linux):
        ...
        with patch('cloudmesh.ai.common.sys.Path.glob') as mock_glob:
            ...
            with patch('cloudmesh.ai.common.sys._get_nvidia_gpu_info', return_value={"gpu.present": False}), \
                ...
                info = ai_sys.get_gpu_info()
                self.assertTrue(info["gpu.present"])
                self.assertEqual(info["gpu.vendor"], "Intel")
    ...
    def test_no_gpu(self):
        ...
        with patch('subprocess.check_output', side_effect=OSError), \
            ...
            info = ai_sys.get_gpu_info()
            self.assertFalse(info["gpu.present"])
